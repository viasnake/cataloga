...
test('Cloudflare packaging writes viewer assets, bundle, and metadata', () => {
  ...
  try {
    ...
    const result = createCloudflarePackage({
      ...
      bundlePath,
      outDir,
      dataRepo: 'example/home-ledra-data',
      dataRef: 'refs/heads/main',
      dataCommitSha: 'abcdef123456abcdef123456abcdef123456abcd',
      engineRepo: 'viasnake/ledra',
      engineRef: 'v0.1.0',
      engineCommitSha: 'fedcba654321fedcba654321fedcba654321fedc',
    ...
    assert.equal(result.metadata.data.repo, 'example/home-ledra-data');
    assert.equal(result.metadata.engine.ref, 'v0.1.0');

    const metadata = JSON.parse(readFileSync(join(outDir, 'metadata.json'), 'utf8'));
    assert.equal(metadata.bundle.path, '/bundle.json');
    assert.match(metadata.deploymentVersion, /^abcdef123456-fedcba654321-20260314T120000Z$/u);
  } finally {
